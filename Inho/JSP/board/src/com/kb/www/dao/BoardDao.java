...
public class BoardDao {
	...
	public int insertMember(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("insert into member(id, pw) value(?, ?)");
	...
	public MemberVo getMember(String id) {
		...
		MemberVo vo = null;
		try {
			pstmt = con.prepareStatement("select sq, id, pw from member where binary(id)=? and leave_fl=false");
			pstmt.setString(1, id);
			rs = pstmt.executeQuery();
			while (rs.next()) {
				vo = new MemberVo();
	...
	public int updateMember(MemberVo vo) {
		...
		int count = 0;
		try {
			pstmt = con.prepareStatement("update member set pw=? where id=?");
